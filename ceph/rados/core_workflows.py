    ...
    def create_inconsistent_object(self, objectstore_obj, pool_name, no_of_objects):
        ...
        try:
            ...
            for object_name in object_list:
                primary_osd = self.get_osd_map(pool=pool_name, obj=object_name)[
                    "acting_primary"
                ]
                pg_id = self.get_osd_map(pool=pool_name, obj=object_name)["pgid"]
            ...
            for osd_id, objects_list in osd_object_map.items():
                for obj_name in objects_list:
                    obj_str = objectstore_obj.list_objects(
                        osd_id=osd_id, pgid=object_pgid_map[obj_name], obj_name=obj_name
                    ).strip()
                ...
                if not self.change_osd_state(action="start", target=int(osd_id)):
                    ...
                    raise Exception(f"Could not stop OSD.{osd_id}")
            ...
            for obj, pg_id in object_pgid_map.items():
